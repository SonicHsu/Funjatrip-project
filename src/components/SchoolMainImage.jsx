import badge from "../assets/images/index/badge.svg";

export default function SchoolMainImage({ photoUrl, school, }) {
  return (
    <div className="w-full h-full">
      ...
      <div className="absolute top-0 left-0 pl-12">
        <div className="relative w-auto h-[100px]">
          <img src={badge} alt="badge" className="object-cover" />

          <div className="absolute inset-0 flex flex-col items-center justify-start text-white mt-2">
            <span className="text-4xl font-bold">{school.travelCount}</span>
            <span>課程</span>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 w-full p-12  text-white ">
        <h3 className="text-5xl font-bold">{school.schoolName}</h3>
        <div className="flex gap-2 mt-2">
          <div className="flex items-center ">
            ...
            <span className=" px-2 py-1 ">{school.areaName}</span>
          </div>
        </div>
        <div className="flex gap-2 mt-2">
          {school?.travelTypeList?.map((type) => (
            <span
              key={type.typeId}
